...
void csvToVideo(char * nomf, VIDEO * tabVideo, int nbElem){
  ...
  if(f!=NULL){
    ...
    for(int i = 0; ret != NULL ; i++){
      ...
      getChamp(ligne,att); //vid_id
      getChamp(ligne,att); //trending_date
      getChamp(ligne,att);
      strcpy(tab[i].titre,att); //title
      getChamp(ligne,att);
      strcpy(tab[i].chaine,att); //channel_title
      getChamp(ligne,att);
      strcpy(tab[i].categorie,att); //category_id
      getChamp(ligne,att); //publish_time
      strcpy(tab[i].motcles,strtok_r(ligne,",",&rest)); //tags
      strcpy(ligne,rest);
      getChamp(ligne,att);
      tab[i].vues = atoi(att); //views
      getChamp(ligne,att);
      tab[i].likes = atoi(att); //likes
      getChamp(ligne,att);
      tab[i].dislikes = atoi(att); //dislikes
      getChamp(ligne,att);
      tab[i].commentaires = atoi(att); //comment_count
